...
- `landing.html` — the complete page (React + Framer Motion, inlined via CDN)
...
Open `landing.html` in a browser — no build step. Note it loads fonts (Fontshare/Google) and React/Framer Motion from CDNs, so it needs an internet connection to render.
